...
export default function SettingsTab({ addToast }: Props) {
  const [settings, setSettings] = useState<Settings>({
    fullscreen: false, default_runner: "Proton", auto_update: "Enabled",
    enable_esync: true, enable_fsync: true, enable_dxvk_async: false, theme: "Dark (Default)",
  });
  ...
  const handleSave = async () => {
    setSaving(true);
    try { await invoke("save_settings", { settings }); addToast("Settings saved!", "success"); }
    catch (e) { addToast(`Failed to save: ${e}`, "error"); }
    finally { setSaving(false); }
  };

  return (
    <div style={{ overflow: "auto", flex: 1 }}>
    <div className="section-label" style={{ marginBottom: 20 }}>Settings</div>
    ...
    <option>Dark (Default)</option><option>Light</option>
    </select>
    <span className="settings-label">Fullscreen Mode</span>
    <select value={settings.fullscreen ? "Enabled" : "Disabled"} onChange={(e) => update("fullscreen", e.target.value === "Enabled")}>
    ...
    <select value={settings.default_runner} onChange={(e) => update("default_runner", e.target.value)}>
    {["Native", "Wine", "Proton", "Flatpak", "Steam"].map((r) => <option key={r}>{r}</option>)}
    </select>
    <span className="settings-label">Auto-check Updates</span>
    <select value={settings.auto_update} onChange={(e) => update("auto_update", e.target.value)}>
    <option>Enabled</option><option>Disabled</option>
    </select>
    <div className="settings-section">Wine / Proton (Global)</div>
    <span className="settings-label" style={{ gridColumn: "1 / -1" }}>
    <label className="checkbox-row">
    <input type="checkbox" checked={settings.enable_esync} onChange={(e) => update("enable_esync", e.target.checked)} />
    Enable Esync globally
    </label>
    </span>
    <span className="settings-label" style={{ gridColumn: "1 / -1" }}>
    <label className="checkbox-row">
    <input type="checkbox" checked={settings.enable_fsync} onChange={(e) => update("enable_fsync", e.target.checked)} />
    Enable Fsync globally
    </label>
    </span>
    <span className="settings-label" style={{ gridColumn: "1 / -1" }}>
    <label className="checkbox-row">
    <input type="checkbox" checked={settings.enable_dxvk_async} onChange={(e) => update("enable_dxvk_async", e.target.checked)} />
