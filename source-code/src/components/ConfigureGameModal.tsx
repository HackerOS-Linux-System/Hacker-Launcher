...
export default function ConfigureGameModal({ game, onClose, onSaved, addToast }: Props) {
  ...
  const [runner, setRunner] = useState(game.runner.includes("Proton") ? "Proton" : game.runner);
  const [protonVersion, setProtonVersion] = useState(game.runner.includes("Proton") ? game.runner : "");
  const [protonVersions, setProtonVersions] = useState<string[]>([]);
  const [fpsLimit, setFpsLimit] = useState(game.fps_limit != null ? String(game.fps_limit) : "");
  ...
  useEffect(() => {
    ...
    .then((p: Array<{ version: string }>) => {
      const versions = p.map((x: { version: string }) => x.version);
  ...
  return (
    ...
    {["Native", "Wine", "Proton", "Flatpak", "Steam"].map((r) => <option key={r}>{r}</option>)}
    </select>
    ...
    <input type="text" value={launchOptions} onChange={(e) => setLaunchOptions(e.target.value)} placeholder="--fullscreen --gamescope ..." />

    <label className="form-label">FPS Limit</label>
    <input type="number" value={fpsLimit} onChange={(e) => setFpsLimit(e.target.value)} placeholder="e.g. 60" />

    {isWineOrProton && (
      <>
      <label className="form-label">Prefix Path</label>
      <input type="text" value={prefix} onChange={(e) => setPrefix(e.target.value)} placeholder="Leave empty for default" />
      <div className="full-row" style={{ display: "flex", flexDirection: "column", gap: 8, paddingTop: 4 }}>
      <label className="checkbox-row">
      <input type="checkbox" checked={enableDxvk} onChange={(e) => setEnableDxvk(e.target.checked)} />
      Enable DXVK / VKD3D
      </label>
      <label className="checkbox-row">
      <input type="checkbox" checked={enableEsync} onChange={(e) => setEnableEsync(e.target.checked)} />
      Enable Esync (Override)
      </label>
      <label className="checkbox-row">
      <input type="checkbox" checked={enableFsync} onChange={(e) => setEnableFsync(e.target.checked)} />
      Enable Fsync (Override)
      </label>
      <label className="checkbox-row">
      <input type="checkbox" checked={enableDxvkAsync} onChange={(e) => setEnableDxvkAsync(e.target.checked)} />
    ...
    <button onClick={onClose} style={{ background: "rgba(255,255,255,0.05)" }}>Cancel</button>
