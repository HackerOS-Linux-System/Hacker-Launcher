...
export default function ProtonsTab({ addToast }: Props) {
  ...
  const fetchVersions = useCallback(async (type: typeof protonType) => {
    ...
    try {
      let versions: string[];
      if (type === "GE") {
        versions = await invoke<string[]>("get_available_ge");
      } else if (type === "Official") {
        versions = await invoke<string[]>("get_available_official", { stable: true });
      } else {
        versions = await invoke<string[]>("get_available_official", { stable: false });
      }
  ...
  const handleTypeChange = (t: typeof protonType) => {
    setProtonType(t);
    fetchVersions(t);
  };
    ...
    if (customSourceType === "Tar.gz File") {
      ...
      if (typeof result === "string") {
        ...
        if (!customVersionName) {
          setCustomVersionName(result.split("/").pop()?.replace(".tar.gz", "") ?? "");
        }
      }
    } else {
      ...
      if (typeof result === "string") {
        setCustomPath(result);
        if (!customVersionName) {
          setCustomVersionName(result.split("/").pop() ?? "");
        }
      }
    }
  };
  ...
  const handleInstall = async () => {
    if (protonType === "Custom") {
      if (!customVersionName || !customPath) {
        return addToast("Name and path are required", "error");
      }
    } else if (!selectedVersion) {
      return addToast("Select a version", "error");
    }
    ...
    try {
      if (protonType === "Custom") {
        if (customSourceType === "Tar.gz File") {
          await invoke("install_custom_tar", {
            tarPath: customPath,
            version: customVersionName,
          });
        } else {
          await invoke("install_custom_folder", {
            srcFolder: customPath,
            version: customVersionName,
          });
        }
        addToast(`Custom Proton "${customVersionName}" installed!`, "success");
      } else {
        await invoke("install_proton", {
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      setSelected(-1);
      addToast(`Removed: ${p.version}`, "success");
      await loadProtons();
    } catch (e) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const result = await invoke<[string, string] | null>("check_proton_update", {
        version: p.version,
        protonType: p.type,
      });
      if (!result) {
        return addToast("No update available", "info");
      }
      ...
      await invoke("remove_proton", { version: p.version });
      addToast(`Updated to ${newVersion}`, "success");
      setSelected(-1);
      await loadProtons();
    } catch (e) {
  ...
  return (
    <>
      ...
      <div className="table-wrap">
        {loading ? (
          <div className="empty-state">
            ...
            <span>Loading protons…</span>
          </div>
        ) : protons.length === 0 ? (
          <div className="empty-state">
            <span>📦</span>
            <span>No Proton versions installed</span>
          </div>
        ) : (
          <table>
            <thead>
              ...
              {protons.map((p, i) => (
                ...
                >
                  ...
                  <td>
                    <span
                      className={`badge ${
                        p.status === "Update Available" ? "badge-yellow" : "badge-green"
                      }`}
                    >
                      {p.status}
                    </span>
      ...
      <div className="btn-row">
        <button onClick={openInstallDialog} disabled={installing}>
          ＋ Install Proton
        </button>
        <button onClick={handleUpdate} disabled={selected < 0 || installing}>
          ↑ Update Selected
        </button>
        <button
          className="btn-danger"
          onClick={handleRemove}
          disabled={selected < 0 || installing}
        >
          ✕ Remove Selected
        </button>
        <button onClick={loadProtons} disabled={loading}>
          ↺ Refresh
        </button>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setShowInstall(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-grid">
              ...
              <select
                value={protonType}
                onChange={(e) => handleTypeChange(e.target.value as typeof protonType)}
              >
              ...
              {protonType !== "Custom" ? (
                <>
                  ...
                  {fetchingVersions ? (
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <span className="spinner" />
                      <span style={{ color: "var(--text-muted)" }}>Fetching versions…</span>
                    </div>
                  ) : (
                    <select
                ...
                <>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCustomSourceType(e.target.value as typeof customSourceType)
                    }
                  >
